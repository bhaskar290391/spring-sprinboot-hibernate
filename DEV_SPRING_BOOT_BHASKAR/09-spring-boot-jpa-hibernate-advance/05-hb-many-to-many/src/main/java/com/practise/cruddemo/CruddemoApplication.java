...
import com.practise.cruddemo.entity.*;
...
public class CruddemoApplication {
	...
	public CommandLineRunner runner(AppDAO dao){
		return  runner->{
			...
			//findCourseAndStudentByCourseId(dao);

			//findStudentsAndCourseByStudnetId(dao);
	...
	private void findStudentsAndCourseByStudnetId(AppDAO dao) {
		int id=2;
		Student student=dao.findStudentsAndCourseByStudentId(id);
		System.out.println("The student ==> "+student);
		System.out.println("The Courses ==> "+student.getCourses());
	}

	private void findCourseAndStudentByCourseId(AppDAO dao) {
		int id=10;
		Course theCourse=dao.findCourseAndStudentByCourseId(id);
	...
	private void createCourseAndStudents(AppDAO dao) {
		...
		Course course=new Course("Bhaskar The great");

		Student s=new Student("bhaskar","maddy","bhaskar@test.com");
		Student s1=new Student("Maddy","Mudaliyar","maddy@test.com");

		course.addStudent(s);
		course.addStudent(s1);
		dao.createCourseAndStudents(course);
	}
